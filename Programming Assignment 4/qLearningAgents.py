...
class PacmanDeepQAgent(PacmanQAgent):
    ...
    def compute_q_targets(self, minibatch, network=None, target_network=None, doubleQ=False):
        ...
        nBatch = len(minibatch)

        Q_predict = network.run(states).data
        Q_target = np.copy(Q_predict)

        b = 1.0 / (2.0 * np.sqrt(self.counts[states_np[:, 0], states_np[:, 1]] / 100.0))

        n = network.run(next_states)
        res_next = target_network.run(next_states)
        agmax = np.argmax(n.data, axis=1)
        qagmax = np.choose(agmax, res_next.data.T)

        q_diff = rewards + (1.0 - dones) * self.discount * qagmax + b
        Q_target[np.arange(nBatch), actions] = q_diff

        "*** YOUR CODE ENDS HERE ***"

        if not (self.td_error_clipping is None):
